...
extension UIAlertController {
    ...
    public func addImagePicker(_ scroll: UICollectionViewScrollDirection, paging: Bool, height: CGFloat? = nil, images: [UIImage], selection: ImagePickerViewController.SelectionType? = nil) {
        ...
        self.setContentViewController(imagePicker)
    }
}

final public class ImagePickerViewController: UIViewController {
    
    public typealias SingleImagePickerAction = (UIImage?) -> Swift.Void
    public typealias MultiImagePickerAction = ([UIImage]) -> Swift.Void
    
    public enum SelectionType {
        case single(SingleImagePickerAction?)
        case multiple(MultiImagePickerAction?)
    }
    
    fileprivate typealias CellConfig = (ItemWithImage?) -> Swift.Void
    ...
    func reloadDataSource() {
        dataSource = data.map { image in
            let config: CellConfig = { cell in//[unowned self] cell in
